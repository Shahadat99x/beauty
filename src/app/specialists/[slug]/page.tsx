...
export function generateMetadata({ params }: { params: { slug: string } }) {
  const specialist = specialists.find((s) => s.slug === params.slug);
...
export default function SpecialistDetailPage({
  ...
  const signatureTreatments = specialist.signatureServices
    ? services.filter((s) => specialist.signatureServices!.includes(s.slug))
    : [];
         ...
         <section className="section-space pt-0">
            <Container>
               <div className="surface-panel rounded-[2rem] p-8">
                  <h3 className="text-xs font-semibold uppercase tracking-[0.24em] text-muted">Core Competencies</h3>
                  <div className="mt-5 flex flex-wrap gap-4">
                     {specialist.specialties.map(specialty => (
                        <span key={specialty} className="rounded-full border border-border/70 bg-white/60 px-5 py-2.5 text-sm font-medium text-ink shadow-[0_4px_12px_rgba(69,54,48,0.03)]">
                           {specialty}
                        </span>
                     ))}
                  </div>
      ...
      {specialist.recentWorkImages && specialist.recentWorkImages.length > 0 && (
         <section className="section-space bg-shell-soft">
            <Container>
               <SectionHeading 
                 eyebrow="Results"
                 title="Clinical & Aesthetic Work"
                 description="A preview of healing progression, subtle shaping, and completed protocols."
               />
               <div className="mt-10 grid gap-6 sm:grid-cols-2">
                 {specialist.recentWorkImages.map((img, idx) => (
                    <MediaFrame 
                       key={idx}
                       aspect={img.aspect}
                       tone={img.tone}
