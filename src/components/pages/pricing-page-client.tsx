...
export function PricingPageClient() {
  ...
  const content = getPricingPageContent(locale);
  const pricingSnippets = getPricingSnippets(locale);
  const pricingCategories = getPricingCategories(locale);
  const faqItems = getPricingFaqs(locale);
        ...
        <Container>
          <div className="flex flex-col gap-20">
            {pricingCategories.map((category) => {
              const isMembership = category.title === "Nuolatinė priežiūra" || category.title === "Ongoing care";
                          ...
                          <ul className="mt-6 flex flex-col gap-3">
                            {pkg.items.map((item) => (
                              <li key={item} className="flex items-start gap-3 text-sm">
                                ...
                                />
                                <span className={pkg.isPopular ? "text-white/90" : "text-ink"}>
                                  {item}
                                </span>
                              </li>
                            ))}
